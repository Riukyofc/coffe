...
const ProductCard = ({ product, index = 0 }) => (
  ...
  >
    <div className="w-full h-[350px] overflow-hidden bg-zinc-950 mb-6 relative rounded-sm">
      ...
      <img src={product.image} alt={product.title} className="w-full h-full object-cover object-center group-hover:scale-110 transition-transform duration-[1.5s] ease-out" />
      <div className="absolute inset-x-0 bottom-0 p-4 translate-y-full group-hover:translate-y-0 transition-transform duration-500 ease-out z-20">
        <button className="w-full bg-[#C5A059]/90 backdrop-blur-md text-black py-3 text-xs font-bold tracking-widest uppercase hover:bg-white transition-colors">Quick Add</button>
      </div>
    </div>
    <div className="flex-grow flex flex-col">
      ...
      <div className="flex justify-between items-center border-t border-zinc-800/50 pt-4 mt-auto">
        ...
        <button className="text-zinc-500 group-hover:text-[#C5A059] transition-colors p-2 bg-zinc-900 rounded-full group-hover:bg-zinc-800">
